...
class access {
	...
	public function isUser() {
		...
		$login		= $session->get('login');
		$date_now	= $this->date->datetimeDbFormat();
		if (isset($login['username']) && isset($login['apanel_user']) && ! $login['apanel_user']) {

			$client_user_query	= $db->setTable('client_user')
									->setFields('username')
									->setWhere("username = '{$login['username']}'")
									->buildSelect(false);

			$gsa_user_query		= $db->setTable('gsa_user')
									->setFields('username')
									->setWhere("username = '{$login['username']}'")
									->buildSelect(false);

			$result = $db->setTable("($client_user_query UNION $gsa_user_query) a")
						->setFields('username')
						->runSelect(false)
						->setLimit(1)
						->getResult();
