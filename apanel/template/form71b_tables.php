			...
			<?php foreach ($direct as $key => $row): ?>
				...
				<?php elseif ($row): ?>
					<?php
						...
						if ($key == 0 || $prev_sector != $row->origin || $prev_sector_d != $row->destination) {
							$sub_entries		= 0;
							$subtotal_numMawbs		= 0;
							$subtotal_weight		= 0;
							$subtotal_fcharge		= 0;
					...
					<?php if ( ! isset($direct[$key + 1]) || $prev_sector != $direct[$key + 1]->origin || $prev_sector_d != $direct[$key + 1]->destination): ?>
						<tr class="info">
...
<div class="table-responsive mb-xs">
	<table id="tableList2" class="table table-hover table-bordered table-sidepad mb-none">
		<thead>
			...
			<tr class="info">
				<th rowspan="2" class="col-md-2">AIR CARRIER/FORWARDER</th>
