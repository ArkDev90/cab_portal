<div class="panel panel-primary br-xs">
    ...
    <div class = "row report_name">
        FORM 51-B : Monthly International Cargo Traffic Flow (January 2018)
    </div>
    <div class = "row nav_link">
        <a href = "">Back to FORM 51-B : Monthly International Cargo Traffic Flow List </a>
    </div>
    ...
    <div class = "row" style = "margin: 2% 0% 0% 0%">
        <table border = "1" id = "direct_cargo" class="table table-hover table-sidepad">
			<?php
				echo $ui->loadElement('table')
						->setHeaderClass('info')
						->addHeader('DIRECT CARGO (Kilograms)', array('colspan' => '11'))
						->draw();
			?>
            <tbody>
    ...
    <div class = "row" style = "margin: 2% 0% 0% 0%">
        <table border = "1" id = "transit_cargo" class="table table-hover table-sidepad">
			<?php
				echo $ui->loadElement('table')
						->setHeaderClass('info')
						->addHeader('TRANSIT CARGO (Kilograms)', array('colspan' => '11'))
						->draw();
			?>
            <tbody>
...
<script>
	...
	function getList() {
		...
		ajax_call = $.post('<?=MODULE_URL?>ajax/ajax_direct_cargo_list', ajax, function(data) {
			$('#direct_cargo tbody').html(data.table);
    ...
    function getList2() {
		...
		ajax_call2 = $.post('<?=MODULE_URL?>ajax/ajax_transit_cargo_list', ajax, function(data) {
			$('#transit_cargo tbody').html(data.table);
			if (ajax.page > data.page_limit && data.page_limit > 0) {
				ajax.page = data.page_limit;
				getList2();
			}
