...
class controller extends wc_controller {
	...
	public function __construct() {
		...
		$this->form71a_model		= new form71a_model();
		...
		$this->report_form_table	= 'form71a';

		$this->report_form_id		= '306';
	...
	public function listing() {
		...
		$this->view->load('form71a/form_71a_list', $data);
	...
	public function create($year = 0, $month = 0) {
		...
		$data['mix_list']			= $this->reporting_model->getOriginDestinationList($this->report_form_id, '');
		...
		$this->view->load('form71a/form_71a', $data);
	...
	public function edit($report_id) {
		...
		$report = $this->form71a_model->getReport($this->header_fields, $report_id, $this->client_id, $this->user_id, $this->user_group);
		...
		$data['form_details']		= $this->form71a_model->getReportDetails($this->detail_fields, $report_id);

		$data['form_details2']		= $this->form71a_model->getReportDetails2($this->detail_fields2, $report_id);

		$data['form_details3']		= $this->form71a_model->getReportDetails3($this->detail_fields3, $report_id);
	...
	public function delete($report_id) {
		...
		$this->view->load('form71a/form_71a_delete', $data);
	...
	public function view_draft_list() {
		...
		$this->view->load('form71a/form_71a_draft_list', $data);
	...
	private function ajax_list() {
		...
		foreach ($pagination->result as $key => $row) {
			...
			if($row->status == "Draft")
			{
				$row_url = BASE_URL ."form_71a/edit/".$row->id;
			}
			else
			{
				$row_url = BASE_URL ."client_users_mgt/users/report_viewer/".$this->client_id."/form71a/".$row->id;
			}
	...
	private function ajax_create() {
		...
		$result = $this->form71a_model->saveReport($header, $details, $details2, $details3);



		if ($result && $status == 'Approved') {

			$report_id = $this->form71a_model->getReportId();
	...
	private function ajax_edit() {
		...
		$result = $this->form71a_model->updateReport($header, $details, $details2, $details3, $report_id, $this->client_id);
	...
	private function ajax_delete() {
		...
		$result = $this->form71a_model->deleteReport($report_id, $this->client_id);
