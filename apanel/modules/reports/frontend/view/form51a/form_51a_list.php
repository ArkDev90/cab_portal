		...
		<table id="tableList" class="table table-hover table-bordered table-sidepad">
			...
			<?php
				...
				echo $ui->loadElement('table')
						...
						->addHeader('Date Created', array('class' => 'col-md-3'))
...
<script>
	...
	$('#report_quarter').on('change', function() {

		ajax.quarter = $(this).val();
	...
	$('#check_report').click(function() {
		...
		ajax_call = $.post('<?=MODULE_URL?>ajax/ajax_check_report', ajax, function(data) {
			...
			if (data.existing) {
				...
				window.location = '<?=MODULE_URL?>create/' + ajax.year + '/' + ajax.quarter;
