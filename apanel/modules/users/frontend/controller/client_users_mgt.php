...
class controller extends wc_controller {
    ...
    public function client_user_list() {
		...
		$result	= $this->client_mgt_model->getClientInfo($this->client_fields, $client_id);

		$data['airline_represented'] = $result->name;
    ...
    public function create_user($client_id) {
        ...
        //$data['user_type']	= array('Form Admin' => 'Form Admin', 'User' => 'User', 'Contributor' => 'Contributor');

$data['user_type']	= array('User' => 'User');
	...
	public function approved_reports() {
		...
		$data['client_id']		= $login['client_id'];

		$data['report_list']	= $this->client_mgt_model->getUsersReportList();



		$this->view->load('client_mgt/approved_reports', $data);

	}



	public function pending_reports($db_table) {
		...
		$this->view->load('client_mgt/pending_reports', $data);

	}



	public function view_approved_reports($db_table) {
	...
	private function ajax_approved_reports() {

		$id = $this->input->post('id');

		$client_id = $this->input->post('client_id');

		$result = $this->client_mgt_model->getUsersReportList($client_id, $id);
		...
		foreach ($result as $key => $row) {
			...
			$table .= '<td><a href="'.MODULE_URL.'view_approved_reports/'.$row->db_table.'">'.$row->title.'</a></td>';
	...
	private function ajax_report_approval() {
		...
		$data = array(
			...
			'approveddate'	=> $this->date->dateDbFormat()

		);



		$report_id		= $this->client_mgt_model->reportApproval($id, $db_table, $data);

		$report_form_id	= $this->client_mgt_model->getReportFormID($db_table);



		if ($report_id) {

			$this->portal->sendReportConfirmation($report_form_id->id, $id);
		...
		return array(
			...
			'redirect'	=> MODULE_URL . 'pending_reports/' . $db_table,

			'success'	=> (boolean) $report_id
	...
	private function ajax_report_disapproval() {
		...
		$result = $this->client_mgt_model->reportApproval($id, $db_table, $data);
	...
	private function ajax_view_approved_reports() {
		...
		$pagination = $this->client_mgt_model->getApprovedReportList($db_table, $month, $year,$client_id, $month, $year, $sort);
		...
		$table .= '<td>'.$row->timespan. ' '. $row->year .'</td>';
			...
			$table .= '<td align="center">'.$row->submittedby.'</td>';

			$table .= '<td align="center">'.$submitteddate.'</td>';

			$table .= '<td align="center">'.$row->approvedby.'</td>';

			$table .= '<td align="center">'.$approveddate.'</td>';

			$table .= '<td style = "text-align:center"><a href = "'.MODULE_URL.'report_viewer/'.$client_id.'/'.$db_table.'/'.$row->id.'">View Approved</a></td>';		
	...
	private function ajax_pending_reports() {
		...
		$pagination = $this->client_mgt_model->getPendingReportList($client_id, $db_table);
		...
		foreach ($pagination as $key => $row) {
			...
			elseif($row->timespan == '1') {$row->timespan = '1st Quarter (Jan,Feb,Mar)';}
			...
			$table .= '<td style = "text-align:center"><a href = "'.MODULE_URL.'report_viewer/'.$client_id.'/'.$db_table.'/'.$row->id.'">Report Viewer</a></td>';
	...
	public function change_level($id) {
		...
		$data['air_type'] = (array) $this->client_mgt_model->getNatureOfOperationList();

		$data['user_types']	= array('Form Admin' => 'Form Admin', 'Summary Viewer' => 'Summary Viewer', 'User' => 'User', 'Contributor' => 'Contributor');
		...
		$data['ajax_task'] = 'ajax_change_level';

		$data['ajax_post'] = "&id=$id";

		$this->view->load('client_mgt/change_level', $data);
	...
	public function delete_user($id) {
		...
		$data['ajax_task'] = 'ajax_delete_user';

		$data['ajax_post'] = "&id=$id";

		$this->view->load('client_mgt/delete_user', $data);
    ...
    private function ajax_user_list() {
		...
		// $login_user = $this->client_mgt_model->getCurrentUser($this->fields, $username);

		// $usertype 	= $login_user->user_type;
		...
		foreach ($pagination->result as $key => $row) {
            ...
            if ($row->username == USERNAME && $row->user_type == 'Master Admin') {

				$table .= '<td style = "text-align:center"><a href = "'.MODULE_URL.'reset_pw/'.$row->id.'"> Reset Password </a> | <a href = "'.MODULE_URL.'account_info/'.$row->id.'">Edit Profile </a> | <a href = "'.MODULE_URL.'change_level/'.$row->id.'"> Change Level </a> | <a href = "'.MODULE_URL.'delete_user/'.$row->id.'"> Delete </a> </td>';

			}

			else if ($row->username != USERNAME && $row->user_type == 'Master Admin') {

				$table .= '<td style = "text-align:center; font-weight:bold; color:gray"> Reset Password | Edit Profile |  Change Level | Delete </td>';

			}

			else if ($row->username != USERNAME && $row->user_type != 'Master Admin') {

				$table .= '<td style = "text-align:center"><span style = "text-align:center; font-weight:bold; color:gray"> Reset Password </span> | <a href = "'.MODULE_URL.'account_info/'.$row->id.'">Edit Profile </a> | <a href = "'.MODULE_URL.'change_level/'.$row->id.'"> Change Level </a> | <a href = "'.MODULE_URL.'delete_user/'.$row->id.'"> Delete </a> </td>';
    ...
    private function ajax_create_user() {
        ...
        return array(
			...
			'redirect'	=> MODULE_URL. 'client_user_list/' .$client_id,
	...
	private function ajax_checked_operation_list() {
		...
		$nature = $this->client_mgt_model->getCheckClientUserNature($id);
	...
	private function ajax_change_level() {
		...
		$result 	= $this->client_mgt_model->updateClientUserLevel($user_type, $nature, $id);
	...
	private function ajax_delete_user() {
		...
		$result = $this->client_mgt_model->deleteClientUser($id);
	...
	private function ajax_view_61a_content() {
		...
		foreach ($pagination as $key => $row) {
			...
			$table .= '<td style = "text-align:right">'.number_format($row->flyTimeHour, 0).'</td>';
		...
		$table .= '<td style = "text-align:right">'.number_format($totalhour, 0).'</td>';
	...
	public function formt1a_pdf($client_id, $id) {
		...
		$get->name = substr($get->name, 0, 50);

		$get->address = substr($get->address, 0, 50);

		$get->email = substr($get->email, 0, 50);
